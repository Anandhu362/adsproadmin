...
import { Mail, Phone, Edit3, Save, X, Briefcase, ShieldCheck, Loader2 } from "lucide-react";
...
const EmployeeDetailsModal = ({ employee, isOpen, onClose, onUpdate }: Props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Success", description: "Profile updated and synced." });
        setIsEditing(false);
          ...
          {!isEditing ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div>
                    ...
                    <span className={`text-xs font-black px-2 py-0.5 rounded-full border mt-1 inline-block ${formData.status === 'Active' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                      {formData.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
              <div className="space-y-2">
                <Label className="font-black text-slate-700 text-xs uppercase">Phone Number</Label>
                <Input value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} className="h-12 border-slate-300 font-bold" />
              </div>
              <div className="space-y-2">
                <Label className="font-black text-slate-700 text-xs uppercase">Job Role</Label>
                <Input value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})} className="h-12 border-slate-300 font-bold" />
              </div>
              <div className="space-y-2">
                <Label className="font-black text-slate-700 text-xs uppercase">Availability Status</Label>
                <Select value={formData.status} onValueChange={(v) => setFormData({...formData, status: v})}>
                  <SelectTrigger className="h-12 border-slate-300 font-bold"><SelectValue /></SelectTrigger>
              ...
              <Button onClick={handleSave} className="w-full bg-slate-900 text-white font-black h-12 text-lg mt-4" disabled={loading}>
                {loading ? <Loader2 className="animate-spin h-5 w-5" /> : <><Save className="mr-2 h-5 w-5" /> Save Changes</>}
              </Button>
            </div>
          )}
        </div>
